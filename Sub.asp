...
'*************************************************************************************************
' 目的：    按商品品牌-显示商品列表
' 输入：    Brand_id,NumsPerPage,NumsPerRown,SortBy,showlist,flag
' 说明：    Brand_id:商品品牌id;  NumsPerPage:每页记录条数;  NumsPerRow:每行显示的商品数量; SortBy表示信息排序;  showlist表示商品显示方式;flag表示商品特性

'*************************************************************************************************
Sub Product_Listbrand(brandid,NumsPerRow,NumsPerPage)
        response.write "<table cellspacing=0 cellpadding=2 style='border-collapse: collapse' width='100%'>"
        response.write "<tr align=center>"
        
        if flag=0 then
       		sql="select id,Product_info_Name,Product_info_PriceM,Product_info_PriceS,Product_info_PicS from Product_info where Product_info_OnOff=0 and product_info_brand="&brandid&" "&SortBy&" "        
        else
       		sql="select id,Product_info_Name,Product_info_PriceM,Product_info_PriceS,Product_info_PicS from Product_info where Product_info_OnOff=0 and instr(Product_info_flag,"&flag&") and product_info_brand="&brandid&"  "&SortBy&" "        
        end if

...
sql="select * from prod_info where prod_info_name='"&prod_info_name&"' and id<>"&id
...
Set rs=Server.CreateObject("ADODB.Recordset")
sql="select * from prod_info where id="&id
rs.open sql,conn,1,3
rs("cid")			   = cid
...
if root_option_PicSType=1 then '手动发布小图片
	if ismodipic=true then rs("prod_info_PicB")=prod_info_PicB
	if ismodipic2=true then rs("prod_info_PicS")=prod_info_PicS
else
	if ismodipic=true then 
...
rs.close
set rs=nothing  
call ok("您已成功修改了一条商品信息！","prod_info_list.asp")
